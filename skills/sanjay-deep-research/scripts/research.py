    ...
    try:
        if args.stream:
            ...
            if result["status"] == "completed":
                ...
                # ALWAYS dump full raw payload to /tmp so we never lose it to expiry
                try:
                    import time as _t
                    _path = f"/tmp/deep_research_{int(_t.time())}.json"
                    with open(_path, "w") as _f:
                        json.dump(result.get("raw", {}), _f, indent=2)
                    print(f"[saved raw -> {_path}]", file=sys.stderr)
                except Exception as _e:
                    print(f"[save failed: {_e}]", file=sys.stderr)
